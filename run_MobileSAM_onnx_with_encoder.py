...
ENCODER_ONNX_MODEL_PATH = f"weights/sam_vit_t_encoder-float.onnx"
DECODER_ONNX_MODEL_PATH = f"weights/sam_vit_t_decoder-singlemask.onnx"
...
class MobileSAMOnnxRunner():
    def __init__(self , encoder_path , decoder_path , model_type , device = "cuda") -> None:
        ...
        assert osp.exists(self.encoder_path) , \
            f"{encoder_path} is not exists"
        assert osp.exists(self.decoder_path) , \
            f"{decoder_path} is not exists"
    ...
    def _inference_encoder(self , encoder_input):
        ...
        print(f"Encoder build image embedding cost time : {(time_end - time_start) * 1000}" , "ms")
    ...
    def _inference_decoder(self , ort_inputs):
        ...
        print(f"Decoder generate mask cost time : {time_cost * 1000}" , "ms")
...
def main():
    image_dir = r"E:\\OroChiLab\\MobileSAM\\images\\test"
    save_dir = r"E:\\OroChiLab\\MobileSAM\\images\\output"
    
    # Get Input Information
    input_point = np.array([[970, 500] , [1200, 730]])
    input_label = np.array([1 , 0]) 
    if USE_BOX :
        input_box = np.array([760,240,1210,730]) # x1 ,y1 , x2 , y2左上角点及右下角点
    
    # Build SAMOnnxRunner
    mobilesam_onnx_runner = MobileSAMOnnxRunner(ENCODER_ONNX_MODEL_PATH , DECODER_ONNX_MODEL_PATH , MODEL_TYPE , DEVICE)
    ...
    for image_file in os.listdir(image_dir):
        ...
        # Inference by onnx
        if USE_BOX:
            masks , iou_predictions , low_res_logits = mobilesam_onnx_runner._inference_img(srcImg , input_point , input_label , input_box)
        else :
            masks , iou_predictions , low_res_logits = mobilesam_onnx_runner._inference_img(srcImg , input_point , input_label)
